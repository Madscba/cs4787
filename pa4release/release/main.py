...
def KerasModel_init(d1, d2, optim="SGD", batch_norm=False, alpha=0.1, beta=0.0, rho1=rho1, rho2=rho2):
    ...
    if not batch_norm:
        ...
        model.add(Dense(units=d1, input_shape=mnist_input_shape))
        model.add(BatchNormalization())
        model.add(Activation(activations.relu))
        model.add(Dense(units=d2))
        model.add(BatchNormalization())
        model.add(Activation(activations.relu))
    ...
    if optim == "SGD":
        ...
        optimizer = keras.optimizers.Adam(
            learning_rate=alpha_adam, beta_1=rho1, beta_2=rho2)
...
def load_MNIST_dataset():
    ...
    Xs_tr = Xs_tr / 255.0
    Xs_te = Xs_te / 255.0
    # 28 rows, 28 columns, 1 channel
    Xs_tr = Xs_tr.reshape(Xs_tr.shape[0], 28, 28, 1)
...
def train_fully_connected_sgd(Xs, Ys, d1, d2, alpha, beta, B, epochs):
    model = KerasModel_init(
        d1, d2, optim="SGD", batch_norm=False, alpha=alpha, beta=beta, rho1=0, rho2=0)
    validation_split = 0.1
    train_history = model.fit(Xs, Ys, batch_size=B,
...
def train_fully_connected_adam(Xs, Ys, d1, d2, alpha, rho1, rho2, B, epochs):
    model = KerasModel_init(d1, d2, optim="Adam", batch_norm=False,
                            alpha=alpha, beta=0, rho1=rho1, rho2=rho2)
    validation_split = 0.1
    train_history = model.fit(Xs, Ys, batch_size=B,
                              epochs=epochs, validation_split=validation_split)
    # model.summary()
...
def train_fully_connected_bn_sgd(Xs, Ys, d1, d2, alpha, beta, B, epochs):
    model = KerasModel_init(
        d1, d2, optim="SGD", batch_norm=True, alpha=alpha, beta=beta, rho1=0, rho2=0)
    validation_split = 0.1
    train_history = model.fit(Xs, Ys, batch_size=B,
                              epochs=epochs, validation_split=validation_split)
    # model.summary()
...
def train_CNN_sgd(Xs, Ys, alpha, rho1, rho2, B, epochs):
    ...
    model.add(tf.keras.layers.Dense(
        num_classes, activation='softmax'))  # dense layer
    opt = tf.keras.optimizers.Adam(
        learning_rate=alpha, beta_1=rho1, beta_2=rho2)
...
def plot_train_val(train_histories, legends, scores_loss, scores_acc, part):
    for i in range(2):
        for j in range(len(legends)):
            ...
            plt.hlines(xmin=0, xmax=len(
                train_histories[j].history[a])-1,  y=score[j], colors='purple', linestyles='dashed')
            plt.legend(['Training {}'.format(["Loss", "Accuracy"][i]), 'Validation {}'.format(
                ["Loss", "Accuracy"][i]), "Final {}: {:.3f}".format(["Loss", "Accuracy"][i], score[j])])
            plt.savefig("Part_{}_{}_{}.png".format(
                part, legends[j], ["Losses", "Accuracies"][i]))
            plt.show()


def part_1(Xs, Ys, Xs_te, Ys_te):
    # part 1.1
    MLP_sgd, train_history_sgd = train_fully_connected_sgd(
        Xs, Ys, d1, d2, alpha, 0, batch_size, epochs)
    score_sgd = evaluate_model(Xs_te, Ys_te, MLP_sgd)
    # print(f'Test loss: {score[0]} / Test accuracy: {score[1]}')
    # part 1.2
    MLP_sgd_mom, train_history_sgd_mom = train_fully_connected_sgd(
        Xs, Ys, d1, d2, alpha, beta, batch_size, epochs)
    score_sgd_mom = evaluate_model(Xs_te, Ys_te, MLP_sgd_mom)
    #
    # #part 1.3
    MLP_adam, train_history_adam = train_fully_connected_adam(
        Xs, Ys, d1, d2, alpha, rho1, rho2, batch_size, epochs)
    score_adam = evaluate_model(Xs_te, Ys_te, MLP_adam)
    #
    # # part 1.4
    MLP_bn_sgd, train_history_bn_sgd = train_fully_connected_bn_sgd(
        Xs, Ys, d1, d2, alpha, beta, batch_size, epochs)
    score_bn_sgd = evaluate_model(Xs_te, Ys_te, MLP_bn_sgd)

    histories = [train_history_sgd, train_history_sgd_mom,
                 train_history_adam, train_history_bn_sgd]
    legends = ["SGD no momentum", "SGD with momentum",
               "Adam", "SGD momentum & batch_norm"]
    scores_loss = [score_sgd[0], score_sgd_mom[0],
                   score_adam[0], score_bn_sgd[0]]
    scores_acc = [score_sgd[1], score_sgd_mom[1],
                  score_adam[1], score_bn_sgd[1]]
    ...
    if optim == "SGD":
        if momentum:
            optimizer = keras.optimizers.SGD(
                learning_rate=alpha, momentum=beta, nesterov=True)
        else:
            optimizer = keras.optimizers.SGD(
                learning_rate=alpha, momentum=0.0, nesterov=False)
    else:
        optimizer = keras.optimizers.Adam(
            learning_rate=alpha_adam, beta_1=0.9, beta_2=0.999)
    model.compile(optimizer=optimizer, loss=keras.losses.sparse_categorical_crossentropy,
                  metrics=[keras.metrics.sparse_categorical_accuracy])
...
def train_fully_connected_sgd2(Xs, Ys, d1, d2, alpha, beta=beta, B=batch_size, epochs=epochs, momentum=False, addi_layer=0, depth=None):
    model = KerasModel_init2(
        momentum=momentum, addi_layer=addi_layer, depth=depth)
    validation_split = 0.1
    train_history = model.fit(
        Xs, Ys, batch_size=B, epochs=epochs, verbose=0, validation_split=validation_split)
    # model.summary()
    # validation_data=(X_test, y_test))
    return model, train_history
...
def train_fully_connected_bn_sgd2(Xs, Ys, d1, d2, alpha, beta, B, epochs, addi_layer=0, depth=None):
    model = KerasModel_init2(optim="SGD", momentum=True, batch_norm=True,
                             alpha=alpha, beta=beta, addi_layer=addi_layer, depth=depth)
    validation_split = 0.1
    train_history = model.fit(
        Xs, Ys, batch_size=B, epochs=epochs, verbose=0, validation_split=validation_split)
    # model.summary()
...
def part_2(Xs, Ys, Xs_te, Ys_te, alpha=0.1, beta=0.9):
    ...
    for alpha in alpha_values:
        MLP_sgd_mom, train_history_sgd_mom = train_fully_connected_sgd2(
            Xs, Ys, d1, d2, alpha, beta, batch_size, epochs, momentum=True)
        val_acc.append(
            train_history_sgd_mom.history["val_sparse_categorical_accuracy"][-1])
    ...
    for additional_layer in additional_layers:
        for additional_layer_depth in additional_layers_depth:
            for beta in beta_values:
                MLP_bn_sgd, train_history_bn_sgd = train_fully_connected_sgd2(
                    Xs, Ys, d1, d2, alpha, beta, batch_size, epochs, addi_layer=additional_layer, depth=additional_layer_depth)
                score_bn_sgd = evaluate_model(Xs_te, Ys_te, MLP_bn_sgd)
                if train_history_bn_sgd.history["val_sparse_categorical_accuracy"][-1] > best_vaL_acc:
                    best_hyperparams = {
                        "extra_layers": additional_layer, "depth": additional_layer_depth, "beta": beta}
    ...
    for i in range(3):
        ...
        for j in range(3):
            additional_layer_depth = int(
                np.random.normal(loc=256, scale=128, size=1)[0])
            for k in range(4):
                ...
                score_bn_sgd = evaluate_model(Xs_te, Ys_te, MLP_bn_sgd)
                combinations_of_hyperparams.append(
                    (additional_layer, additional_layer_depth, beta))
                if train_history_bn_sgd.history["val_sparse_categorical_accuracy"][-1] > best_vaL_acc:
                    best_hyperparams = {
                        "extra_layers": additional_layer, "depth": additional_layer_depth, "beta": beta}
                    best_vaL_acc = train_history_bn_sgd.history["val_sparse_categorical_accuracy"][-1]
...
def plot_metric(tr, val, te, title, metric, epochs):
    ...
    pyplot.plot(tr, label="Training " + metric +
                ", final = " + "{:.3f}".format(tr[-1]))
    pyplot.plot(val, label="Validation " + metric +
                ", final = " + "{:.3f}".format(tr[-1]))
    pyplot.plot([te] * epochs, label="Test " + metric +
                ", final = " + "{:.3f}".format(tr[-1]))
...
def part_3(Xs_tr, Ys_tr, Xs_te, Ys_te):
    model, history = train_CNN_sgd(
        Xs_tr, Ys_tr, alpha_adam, rho1, rho2, batch_size, epochs)
    train_plot(Xs_te, Ys_te, model, history,
