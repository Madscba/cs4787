from pa3release.main import part_1,part_2,part_3,load_MNIST_dataset
import numpy as np
# #!/usr/bin/env python3
...
# def multinomial_logreg_loss(Xs, Ys, gamma, W): #todo Regularization is missing
#     (d, n) = Xs.shape                           #todo check axis
...
#     # TODO students should implement this
#     res = []
#     w_prev = copy.deepcopy(W0)
#     v_prev = copy.deepcopy(W0)
#
#     for i in tqdm(range(num_epochs)):
#         if (i % monitor_period == 0):
#             res.append(copy.deepcopy(w_prev))
#         v_cur = w_prev -alpha * multinomial_logreg_total_grad(Xs, Ys, gamma, w_prev)
#         w_cur = v_cur + beta * (v_cur-v_prev)
#
#         v_prev = v_cur
#         w_prev = w_cur
#
...
if __name__ == "__main__":
    ...
    #     t += 1
    #     sgd_mss_w = sgd_minibatch_sequential_scan(Xs, Ys, gamma, W, alpha, B, 1, monitor_period)
    #     s = rho1 * s + (1-rho1)*sgd_mss_w
    #     r = rho2*r + (1-rho2)*sgd_mss_w**2
    #     s_unbias = s/ (1-rho1**t)
    #     r_unbias = r / (1-rho2**t)
    #     W = W - alpha / np.sqrt(r_unbias+eps) * s_unbias
