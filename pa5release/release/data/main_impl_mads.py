...
def bayes_opt(objective, d, gamma, sigma2_noise, acquisition, random_x, gd_nruns, gd_alpha, gd_niters, n_warmup,
    ...
    for i in range(n_warmup, num_iters):
        if i == n_warmup:
            Xs = tf.squeeze(tf.transpose(xs), 0)
        ...
        for j in range(gd_nruns):
            ...
            obj_min, x_min = gradient_descent(objective, x_optimal, gd_alpha, gd_niters)
            if obj_min <= acq_best:
                x_star = x_min
                acq_best = obj_min
    ...
    return y_best, x_best, tf.convert_to_tensor(xs, dtype=tf.float64), tf.convert_to_tensor(ys, dtype=tf.float64)
    # TODO students should implement this
